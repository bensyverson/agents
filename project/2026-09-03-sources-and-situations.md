...
*2026-09-03.* Two changes to how `agents` works, settled in a design conversation with the maintainer the same day; this doc is the record of what was decided and the work it implies. Imported 2026-09-03 as umbrella `hZ0UQe`.
...
8. **Cache and fetch.** Sources are fetched by shelling out to `git` into `$XDG_CACHE_HOME/agents/sources/<hash of URL>` (`$AGENTS_CACHE` overrides). `sync` and `check` never move a ref; `sync` fetches a source only when the cache lacks it, and `check` fails with a one-line "run `agents sync`" when it does.
